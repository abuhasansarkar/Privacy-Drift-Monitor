...
export interface ResolveOptions {
  /** Plan entitlement. `false` forces our default brand (§6.9). */
...
export async function resolveBranding(
  ...
  const agencyName = agency?.name ?? "Privacy Drift Monitor";

  const branding: Branding = !options.whiteLabelEnabled
    ? defaultBranding(agencyId, agencyName)
    : {
        agencyId,
