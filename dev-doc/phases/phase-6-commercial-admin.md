...
> **Dependencies:** Phase 5 · **Status:** ✅ Complete
...
- [x] Payment failure moves the agency to read-only for scanning while leaving all data visible
- [x] A downgrade over-limit triggers grace, not deletion
- [x] Usage counters are accurate under concurrency
- [x] The free scanner enforces every abuse control
- [x] The free-scan queue cannot starve the paid queue
- [x] An admin can retry a failed job and add a tracker vendor
- [x] Retention deletes expired evidence but **never evidence attached to an open issue**
- [x] Feature flags resolve: agency override → plan targeting → percentage rollout → global default
